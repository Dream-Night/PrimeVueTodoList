<template>
    <div>
    <DataTable :value="dailyFilterdTasks" showGridlines stripedRows tableStyle="win-width: 50rem">
        <Column field="status" header="Status">
            <template #body="slotProps">
            <div v-if="dayChecker(slotProps.data.deadline)">
                <Button severity="warn" v-if="slotProps.data.status.id == 0" icon="pi pi-expand" @click="updateStatus(slotProps.data.id, slotProps.data, slotProps.data.status)" />
                <Button severity="warn" v-if="slotProps.data.status.id == 2" icon="pi pi-play" @click="updateStatus(slotProps.data.id, slotProps.data, slotProps.data.status)" />
                <Button severity="warn" v-if="slotProps.data.status.id == 3" icon="pi pi-check-circle" @click="updateStatus(slotProps.data.id, slotProps.data, slotProps.data.status)" />
                <Button severity="warn" v-if="slotProps.data.status.id == 1" icon="pi pi-replay" @click="updateStatus(slotProps.data.id, slotProps.data, slotProps.data.status)" />
            </div>
            <div v-else>
        ...
        <Column field="formattedDeadlineTime" header="Time/Remaining days">
            <template #body="slotProps">
                <div v-if="dayChecker(slotProps.data.deadline)">Today / {{ slotProps.data.formattedDeadlineTime }}</div>
                <div v-else>{{ howFarFromDeadlineChecker(slotProps.data.deadline) }} days remaining</div>
            </template>
        </Column>
...
import { getTask, removeTask, updateTask, dayContainChecker, dayChecker, howFarFromDeadlineChecker } from './Util/FuncPort.js';
...
const dailyFilterdTasks = computed(() => {
    return (tasks.value.filter(task => dayContainChecker(0, task.costDays) == true));
});
